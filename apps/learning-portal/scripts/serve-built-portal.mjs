...
let control;
if (args["control-socket"] || args.challenge || args["run-id"]) {
  if (!args["control-socket"] || !/^[0-9a-f]{64}$/.test(args.challenge ?? "") || !/^[0-9a-f]{32}$/.test(args["run-id"] ?? "")) throw new Error("CONTROL_ARGUMENT_INVALID");
  const controlPath = path.resolve(args["control-socket"]); const parent = await fs.lstat(path.dirname(controlPath));
  ...
  control = net.createServer((socket) => { let raw = ""; socket.setEncoding("utf8"); socket.on("data", (chunk) => { raw += chunk; if (raw.length > 512) socket.destroy(); }); socket.on("end", () => { let value; try { value = JSON.parse(raw); } catch { socket.end('{"ok":false,"code":"CONTROL_PROTOCOL_INVALID"}\n'); return; } const supplied = Buffer.from(String(value.challenge ?? "")); const expected = Buffer.from(args.challenge); const authenticated = supplied.length === expected.length && crypto.timingSafeEqual(supplied, expected); if (value.version !== "portal-control-v1" || value.runId !== args["run-id"] || !authenticated || !["status", "shutdown"].includes(value.action) || Object.keys(value).sort().join(",") !== "action,challenge,runId,version") { socket.end('{"ok":false,"code":"CONTROL_CHALLENGE_STALE"}\n'); return; } if (value.action === "status") { socket.end(JSON.stringify({ ok: true, status: "running", url: `http://127.0.0.1:${server.address().port}` }) + "\n"); return; } socket.end('{"ok":true,"status":"shutting-down"}\n', () => server.close(() => control.close(async () => { await fs.rm(controlPath, { force: true }); process.exit(0); }))); }); });
