...
async function readState() {
  ...
  if (!value || Object.keys(value).sort().join(",") !== STATE_KEYS.join(",") || value.schemaVersion !== "portal-lifecycle-state-v2" || value.maturity !== "static-portal-stage-a" || !/^[0-9a-f]{32}$/.test(value.runId) || !/^[0-9a-f]{64}$/.test(value.challenge) || !/^http:\/\/127\.0\.0\.1:\d+$/.test(value.url) || !/^[0-9a-f]{64}$/.test(value.buildInventorySha256) || !validateIdentity(value.control) || !validateIdentity(value.controlRoot) || !validateIdentity(value.log) || !validateIdentity(value.run)) throw new Error("STATE_FILE_UNSAFE");
  ...
  return { value, stat: pinned.stat, runDirectory, controlDirectory, controlPath: path.join(controlDirectory, "control.sock"), logPath: path.join(runDirectory, "portal.log") };
}
...
async function control(record, action) {
  ...
  const request = JSON.stringify({ version: "portal-control-v1", action, runId: record.value.runId, challenge: record.value.challenge });
  ...
  if (action === "status" && (value.status !== "running" || value.url !== record.value.url || Object.keys(value).sort().join(",") !== "ok,status,url")) throw new Error("CONTROL_PROTOCOL_INVALID");
  if (action === "shutdown" && (value.status !== "shutting-down" || Object.keys(value).sort().join(",") !== "ok,status")) throw new Error("CONTROL_PROTOCOL_INVALID");
  return value;
}
...
async function start() {
  ...
  const controlPath = path.join(controlDirectory, "control.sock"); const logPath = path.join(runDirectory, "portal.log");
  ...
  let child;
  try {
    child = spawn(process.execPath, [path.join(import.meta.dirname, "serve-built-portal.mjs"), "--control-socket", controlPath, "--challenge", challenge, "--run-id", runId], { cwd: repo, detached: true, stdio: ["ignore", logHandle.fd, logHandle.fd], env: childEnvironment });
  ...
  } catch (error) { await cleanupDirectChild(child); await fs.rm(controlPath, { force: true }); await fs.rm(logPath, { force: true }); await fs.rmdir(controlDirectory).catch(() => {}); await fs.rmdir(runDirectory).catch(() => {}); await removeEmptyParents(); throw error; }
}
...
async function down() {
  ...
  await fs.rm(record.logPath); await fs.rm(statePath); await fs.rmdir(record.controlDirectory); await fs.rmdir(record.runDirectory); await removeEmptyParents();
