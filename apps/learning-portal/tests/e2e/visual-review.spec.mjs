...
test("bounded deterministic Stage A visual evidence", async () => {
  ...
  for (const viewport of [{ name: "desktop", width: 1280, height: 800 }, { name: "narrow", width: 360, height: 800 }]) {
    const context = await chromium.launchPersistentContext("", { channel: "chrome", headless: true, viewport }); const page = await context.newPage(); page.on("console", (message) => { if (message.type() === "error") consoleErrors.push(message.text()); });
    ...
    const result = await new AxeBuilder({ page }).analyze(); axe.push({ viewport: viewport.name, violations: result.violations }); await context.close();
  }
  const noJs = await chromium.launchPersistentContext("", { channel: "chrome", headless: true, javaScriptEnabled: false }); const page = await noJs.newPage(); for (const route of ["/", "/module", "/lesson/promotion-trust", ...["frame", "inspect", "run", "fail", "trace", "decide", "reset", "configure", "verify", "reflect"].map((id) => `/lesson/promotion-trust/step/${id}`)]) { const response = await page.goto(`${baseURL}${route}`); inventory.push({ route, status: response.status(), decision: await page.locator("body").getByText(/decision=insufficient-evidence/).count() }); } await noJs.close();
