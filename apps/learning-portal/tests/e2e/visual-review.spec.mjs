...
test("bounded deterministic Stage A visual evidence", async () => {
  await prepareEvidenceRoot(evidence, { reset: true });
  ...
  try {
    ...
    for (const viewport of [{ name: "desktop", width: 1280, height: 800 }, { name: "narrow", width: 360, height: 800 }]) {
      ...
      for (const state of [{ name: "catalog", route: "/" }, { name: "grains", route: "/lesson/promotion-trust/step/inspect" }, { name: "decision", route: "/lesson/promotion-trust/step/decide" }, { name: "unavailable", route: "/lesson/promotion-trust/step/verify" }]) {
        ...
        const name = `${viewport.name}-${state.name}.png`; const temporaryPath = path.join(temporary, name); await page.screenshot({ path: temporaryPath, fullPage: true }); await writeOwnedEvidence(name, await fs.readFile(temporaryPath), evidence);
    ...
    await noJs.close();
    await writeOwnedEvidence("axe.json", JSON.stringify(axe, null, 2), evidence);
    await writeOwnedEvidence("no-js-inventory.json", JSON.stringify(routeInventory, null, 2), evidence);
    await writeOwnedEvidence("console-csp.json", JSON.stringify({ consoleErrors, csp: "strict-self-no-connect" }, null, 2), evidence);
    await writeOwnedEvidence("dom-inventory.json", JSON.stringify(domInventory, null, 2), evidence);
