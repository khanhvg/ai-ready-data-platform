...
from learning.curriculum.tools.architecture_expansion import clean_handoff
...
class SecurityAndBoundsTest(unittest.TestCase):
    def test_valid_controls_and_named_mutations(self):
        families = json.loads(FIXTURE.read_text())["families"]
        selected = [item for item in families if item["entrypoint"] == "I11-EP-HANDOFF"]
        self.assertEqual(4, len(selected))
        for family in selected:
            self.assertEqual((), clean_handoff(normalize_request(family["validControl"])).codes)
            for mutation in family["mutations"]:
                with self.subTest(family=family["id"], mutation=mutation["mutationId"]):
                    result = clean_handoff(normalize_request(_replace(family["validControl"], mutation["replace"])))
