...
from learning.curriculum.tools.check_traceability import run
...
class TraceabilityTest(unittest.TestCase):
    def test_valid_controls_and_named_mutations(self):
        ...
        selected = [item for item in families if item["entrypoint"] == "I11-EP-TRACE"]
        self.assertEqual(4, len(selected))
        for family in selected:
            self.assertEqual((), run(normalize_request(family["validControl"])).codes)
            for mutation in family["mutations"]:
                with self.subTest(family=family["id"], mutation=mutation["mutationId"]):
                    result = run(normalize_request(_replace(family["validControl"], mutation["replace"])))
