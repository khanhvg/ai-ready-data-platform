...
- Orchestration (`orchestration/airflow/`): flat DAG `seed → load_raw → dbt_run → dbt_test (+ optional publish_iceberg)`, PythonOperators over `callables/pipeline.py`.
...
- **Post-plan label flow:** on completion, remove `triaged`, add `ready for plan audit`.
- **Next stage:** `/ck:cook` on this plan **after** the plan audit — not part of this stage.
