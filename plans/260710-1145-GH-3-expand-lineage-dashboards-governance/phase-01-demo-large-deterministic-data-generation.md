...
- **Bounded per-order summary index (the key structure):** the only cross-table state kept in memory is a compact per-order summary needed by downstream facts — `order_id → (customer_id, store_id, order_date, status, item_count)` — NOT the full order/item rows. Orders are generated in a single seeded loop that (a) streams `orders` rows, (b) streams the matching `order_items` rows immediately, (c) appends the small summary tuple to the index. `payments`, `returns`, and `shipments` are then streamed by iterating the summary index (payments for all orders; returns/shipments only for `status in (completed, returned)`), so they reference real orders without holding item rows. The index is O(orders) of small tuples, bounded and measured against the memory target.
...
- **Determinism ordering:** one `random.Random(seed)` instance, drawn in a fixed table/row order (dimensions → orders+items → payments → returns → shipments → sessions → events). Any new draw is appended at the end of its table's sequence so existing-within-issue-#3 sequences stay stable across runs.
