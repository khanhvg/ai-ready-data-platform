...
| RUN-CMD-01 | Exactly eight typed command IDs resolve from the released Issue #8 contract; no raw command/argv/env/cwd/path/URL override | registry/type/unknown-field/property tests; lock/hash artifact | PH-C05; SC-02; lesson command contract | Dependency STOP; disable runner on mismatch |
...
| RUN-RES-01 | 16 GiB policy enforces wall/CPU/RSS/disk/file/FD/process/output caps and complete descendant cleanup | CPU spin, memory, sparse/disk, file-size, FD, output flood, TERM-ignore, grandchild/setsid tests | PH-H01; PH-C05; SC-04 | TERM/KILL/reap, no state/pointer advancement |
...
| THR-RES-01 | CPU/memory/disk/FD/output/process bomb; descendant escapes group | rlimits + 100 ms tree monitor + PID/start time + TERM/KILL/reap | bounded typed failure; no remaining descendant; pointer unchanged | 100 ms sampling permits bounded transient overshoot |
...
| Process-tree RSS/CPU monitor misses a fast escape | Low / High | rlimits, 100 ms monitor, PID/start tracking, escaped-session tests, whole-tree postcheck | quota/descendant evidence |
...
| Quotas harm 16 GiB laptop or are too small for dbt | Medium / High | one global mutation, explicit caps, RED/real bounded run, Phase 8 may later revise via owner decision | resource manifest |
