...
50-path absence, and 33/21 byte equality. Independent validation and separate readiness have
passed; the cook still replays this preflight and cannot pass from a label or stale report.
...
- The exact 16-command runtime contract has one private `$I11_RUNTIME` candidate, uses
  `$I11_RUNTIME/venv/bin/python` for every I5-06 workload, preserves the frozen released Make
  launchers, defines the exact `env -i` table, and has no undocumented released-command byproduct
  layout.
...
3. Parse exact 50/16 catalogues, literal-admit the controller environment and actual released
   byproduct paths, and prove the 50 paths absent.
...
6. Bind the independent validation and tracked readiness result; stop before product writes if
   either exact pushed identity, report, or external attestation differs.
