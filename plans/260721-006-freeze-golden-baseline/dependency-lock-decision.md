...
### Non-circular compiler-lock materialization

The compiler lock is materialized as reviewed source bytes before any compiler package is
installed. This is the exact 40-line content of
`requirements/golden-lock-tools.lock`; the path in its generated header is semantic because it is
part of the accepted byte fingerprint:
...
#
#    pip-compile --allow-unsafe --generate-hashes --no-emit-index-url --no-emit-trusted-host --output-file=requirements/golden-lock-tools.lock --pip-args='--only-binary=:all: --no-cache-dir --index-url https://pypi.org/simple' --strip-extras -
#
--only-binary :all:
...
The cook writes tests first, then materializes exactly these bytes with the repository editing
mechanism, verifies 40 lines and SHA-256 `ece1d206…`, and only then lets CPython's bundled pip
install the compiler lock. It must not use an ambient `pip-compile`, `uvx`, `npx`, curl-to-shell,
or a temporarily unhardened compiler environment. The installed compiler then regenerates the
840-line application lock from the seven-root `.in`; any byte other than `f41c727b…` is a STOP.
...
Compiler reproduction, application wheel download and each empty-cache install are explicit
networked steps. They use only `https://pypi.org/simple`, have the handoff's time/output/disk
bounds, and retain the first failure. At most one owner-approved retry may run the identical
command/lock in a new private root; no retry may change an index, version, hash, binary policy or
resolver flag.
