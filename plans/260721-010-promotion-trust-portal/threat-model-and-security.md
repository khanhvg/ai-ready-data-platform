...
files, browser storage or network leakage, stale/reused PID, symlink/hardlink aliasing, cleanup
...
| Cleanup → runtime root | nonce/marker/PID/start-identity owned state | repository, home, evidence, other worktrees/processes, aliases |
...
| PTP-S3-07 | File traversal/type confusion | admitted regular build files; containment, media, size, alias checks | `..`, ambiguous decode, symlink/hardlink escape, FIFO/device/socket denied |
...
| PTP-S3-10 | Sensitive/unbounded output | closed errors, redaction, exact file/log/artifact ceilings | canary/private path/flood absent from UI/log/review output |
...
| PTP-S3-12 | Cleanup overreach | namespace marker, nonce, PID/start identity, containment | stale/reused PID, foreign path/process, alias root rejected |
...
It accepts exact Host values for that selected endpoint, GET and HEAD only, no body, request
target at most 2048 bytes, and only a closed generated route/file map. It does not trust
...
- review: two viewports, at most eight named screenshots, one trace, one axe JSON, one no-JS
  inventory, one console/CSP record, one hash manifest;
- text logs at most 2 MiB, binary artifacts at most 16 MiB, all review output at most 64 MiB.

Missing enforcement or measurement is failure. Logs/artifacts never become learner execution or
completion evidence.
...
- exact process, time, file, byte, log, browser-artifact, and cleanup bounds;
...
- Retain RED/GREEN records, dependency binding, unit/contract/S3/a11y results, two-viewport trace
  and screenshots, axe/no-JS/console/CSP reports, artifact hash manifest, cleanup/rollback result,
  and the unapproved human checklist through review policy.
- `learn-down` verifies PID/start identity and owned marker, stops only the static portal, is safe
