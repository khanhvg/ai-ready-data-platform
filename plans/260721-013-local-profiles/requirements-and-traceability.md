...
| FR-05 | Require configured bounds | Every service has memory, CPU, PID, disk, log, health/exit/subprocess timeout, port and volume/project ownership |
...
| FR-10 | Publish strict evidence | `.artifacts/evidence/local-profiles/<run-id>/` contains exact authorities, normalized host/engine/image/config/tool inputs, commands/results, raw+summary measurements, rollback/teardown, strict locators/hash index/completion |
...
| NFR-06 | Bounded execution | Hard timeouts, PID/disk/log caps, zero acceptance restarts, bounded teardown |
...
|---|---|---|---|
| Planning now | Input SHA only | Plan/static/link/dependency/hash/placeholder checks; commit/push plan; issue transition | Independent plan validation requested |
...
| Release handoff | Stage A/B evidence, dependency and golden blast radius, human review | Bounded docs and Issue #5 Phase 13 handoff | No protected drift; exact rollback; ready for separate release process |
...
| FR-05/06 | `LP-BUDGET-LIMIT-OMISSION-006`, `LP-SEC-LOG-UNBOUNDED-010`, `LP-SEC-PID-UNBOUNDED-011`, `LP-READY-TIMEOUT-MISSING-012`, `LP-ADM-GUARDED-PAIR-013` | 2, 3 | `make compose-security-check profile-budget-check` | `static/budgets.json`, `static/security.json` |
...
| FR-09 | `LP-RECOVERY-FOREIGN-SENTINEL-016`, port/volume collision IDs | 2, 5 | `make recovery-test` | `recovery/{ownership,teardown,residue,rollback}` |
...
- Implementing or validating this plan in the planner session.
