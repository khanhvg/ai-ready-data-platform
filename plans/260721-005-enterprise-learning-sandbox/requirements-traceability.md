...
- A TBC is a blocker only for the boundary named in its row. AWS budget/RTO TBCs do not block the
  local portal.
- Planned commands use the normative owner/availability/tier/status/evidence registry in
  `execution-authority-and-release-contract.md`. I5-01 owns the root Make include/help surface;
  later issues own disjoint included fragments.
...
| OWN-02 | First accepted product is the credential-free promotion-trust journey using a grain-honest four-mart evidence bundle | Product/curriculum; P2-P5; I5-02..I5-05 | `make learn-preview` provides only fixture-labelled review; `make learn` reaches controlled failure→diagnose→reset→verify→evidence through real runner/data without false cross-mart attribution or optional credentials | `make lesson-e2e LESSON=promotion-trust`; `make local-journey-e2e`; browser/a11y/evidence artifacts | Static preview/direct expert tools; disable runner; revert lesson/contract version | Tracked P1 fixture, ADR-005, P3 contracts, P4 security; P5 before P6/P7 |
| OWN-03 | Original web-learning experience inspired by, but not copied from, 200ms.thenodebook.com | Web/product; P2/P5; I5-02/I5-05 | Candidates follow the 14-hour kill rule and identical reversible lesson; early preview cannot complete; source/non-copy inventory and reproducible candidate artifacts are retained | `docs/decisions/evidence/adr-0005-web-stack-scorecard.{md,json}`; shared Playwright/a11y suite; manual visual/source review | No ADR on must-gate failure; keep preview; retain sources/locks until I5-05 merge | Merged tracked P1 fixture; real browser gate; ADR-005 blocks P5 |
...
| OWN-07 | Later AWS non-applying design: configurable ap-southeast-1, weekday 08:00-18:00 Asia/HCM, ECS/EC2, S3/Iceberg, ClickHouse/Superset/OpenMetadata | AWS platform; P9-P11; I5-09..I5-11 | Static/offline/mocked checks are labelled honestly; durable authority, readiness, residual cost, S3/IAM/state security, restore/rebuild, composition, and local/AWS divergences are explicit; no resource exists by implication | `make aws-decision-check terraform-check terraform-validate-offline terraform-test-mocked aws-adapters-contract aws-composition-check engine-equivalence` | Reject adapter/topology; local track remains; revoke future role/restore state only under separate runbook | Six apply gates, disposable-validation authority, and exact apply envelope; does not block local core |
...
| NFR-04 | NFR | Deterministic exact-SHA corruption detection plus fresh authority verification; hosted signing separately gated | Every completion/release |
...
| BO-04, CAP-04 | FR-05/06, NFR-04, ASR-01 | All scoped views | ADR-002/003 plus register | P1/P3/P6/P13; I5-01/I5-03/I5-06/I5-13 | `make api-contracts-check`; `make architecture-check`; `make architecture-lab-e2e`; `make traceability-check` | Operation matrix, render manifest, runnable architecture-lab and fitness summary |
...
| Terraform static | `make terraform-check`; `make terraform-validate-offline`; `make terraform-test-mocked` | fmt/validate/lint/security/policy and explicitly mocked tests pass; no deployability claim or apply default |
| Terraform plan | later `make terraform-plan-aws` | Real provider plan is credential/account/role gated, redacted and distinct from apply approval |
...
| Browser/visual/E2E | `make portal-e2e`; `make lesson-e2e`; `make local-journey-e2e`; `make portal-visual-review` | Real runner browser journey and approved screenshots across viewports/states |
...
| Resource | `make profile-budget-check` | Repeated normalized evidence meets owner-approved thresholds; candidates/TBCs cannot pass; invalid combinations refused pre-start |
...
| PH-C03 | AWS persistence / I5-09/I5-10/I5-11 | State matrix and Terraform ownership cover every authority, keys/config, backup/rebuild, RTO/RPO, zero behavior and cost | `make state-matrix-check`; `make aws-composition-check`; later authorized `make aws-restore-drill` | Known-good backup/catalog/key/config versions; local track retained | Apply blocked by ADR-010..014 and validation-environment authority |
| PH-C04 | FinOps/operations / I5-09/I5-10 | Current-source topology-reconciled active/off-hours/failure BOM; durable CostGuard; actual readiness/scale lag and residual services disclosed | `make cost-model-check`; later office open/close/resource inventory | Enforcing block/drain/teardown/reconcile state machine; no scale-to-zero claim; local fallback | Monthly ceiling/retention/cold-start TBC block apply |
| PH-C05 | Runner security / I5-04 | Pinned execution/OS containment, typed allow-list, TOCTOU-safe workspace, browser request boundary, no ambient credentials/network/apply | `make runner-security-test`; import/startup-hook/path-race/CSRF/rebinding/output/secret/base-tree negative suite | Disable runner; static lesson and direct expert tools remain | Local identity default accepted; host lacking containment uses disabled runner |
| PH-C06 | Shared-core + runner / I5-03/I5-04/I5-07 | One state/completion authority, cross-entrypoint fencing, idempotent reset and eleven-asset atomic release/evidence | `make runner-race-test`; Make/Airflow/API barrier/fault/replay/crash suite | Journal/resume, atomic current-release pointer, orphan quarantine/reconciliation | Evidence/authority contract precedes runner |
...
| PH-C08 | Product/curriculum / I5-02/I5-05 | Reviewable preview is non-completing; accepted promotion-trust journey is accessible, runner-backed and grain-honest | `make learn-preview`; `make lesson-e2e LESSON=promotion-trust`; novice/challenge/manual a11y | Static lesson fallback; content version rollback | Tracked real fixture, stack/contract/runner gates |
| PH-C09 | Terraform / I5-10 | Encrypted/versioned/locked state, separate recovery/key authority, and single-use saved-plan envelope bind every exact input; no apply default | `make terraform-check terraform-validate-offline terraform-test-mocked`; mock denial/replay; later real plan/backend restore | Restore prior versions/keys; revoke roles; separate bootstrap/recovery | Account/apply approver TBC; apply blocked |
...
| ID | Owner / issue | Acceptance criterion | Verification | Mitigation / rollback | Dependency / blocker |
|---|---|---|---|---|---|
| PH-H01 | Local runtime / I5-08 | Docker-free core passes; portal+browser+admitted optional profile fits owner-approved normalized thresholds; invalid combinations fail before start | `make profile-budget-check` repeated cold/warm attributed RSS/CPU/disk/network/readiness | Core/no-container fallback; automatic teardown | Candidate thresholds cannot pass until owner-approved |
...
| PH-H11 | Evidence / I5-01/I5-13 | Evidence captures env/tools/commands/SHA/verifier and is CI-retainable/corruption-detecting; local same-host non-repudiation is not claimed | `make evidence-contracts-check`; retention/tamper/live-verifier/clean rerun | Preserve raw prior evidence; never overwrite run; hosted signing later | PH-C02 and local threat model |
...
| PH-H14 | Portal / I5-02 | Unscored preview starts early; candidates obey equal real-fixture tests, per-candidate kill rules, and executable retention; winner recorded before portal build | `make learn-preview`; `make web-spike-scorecard-check`; bundle/RSS/start/a11y/E2E artifacts | No winner on cap/must-gate failure; retain candidates through I5-05 | Merged tracked fixture and fixed first narrative |
...
| SC-16 forged completion / High | Evidence/progress; I5-03/I5-05, P3/P5 | Portal rejects edited state/evidence/hash mismatch and accepts completion only from a fresh private-runner result; same-host owner forgery is outside local cryptographic assurance and never described as non-repudiation | Tamper fixtures, stale/orphan result and browser completion attempts | Reject/audit; replay live verifier; hosted key authority later | Evidence canonicalization, authority transaction and explicit local threat scope |
