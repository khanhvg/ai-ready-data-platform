...
- approved plan/validation/audit SHA;
...
I5-03 may author contract changes only through the shared-core owner. Other issues consume
released versions and must not edit the same contract in parallel. Contract changes use
additive-first versioning, compatibility tests, and a migration note.
...
| 1 | I5-03, I5-04, I5-05 | Sequential contract→runner→portal integration; I5-05 UI work may start after I5-02 but final merge waits for runner | Wave exits only with real promotion-trust E2E |
| 2 | I5-06, I5-07 | Parallel after contract; distinct architecture/content vs data-lab ownership | Shared contract changes routed through I5-01 owner |
...
After validation/audit, create an integration branch from the exact approved plan commit:
...
feature/issue-5-03-learning-contracts
...
1. Rebase/merge current integration branch before final verification.
2. Re-run the issue's golden/contract blast-radius commands.
3. Merge only through a reviewed PR; preserve `risk:high` on the epic.
4. Delete no old contract until dual-read/rollback acceptance passes.
5. Never force-push shared integration/main.
...
| I5-03 | Lesson/lab/progress schemas and first manifests under shared-core review | I5-01 evidence core, I5-02 score |
...
| I5-11 | `platform/adapters/aws/**`, AWS image/config adapters and contract tests | Data contracts + I5-09 state decisions + Terraform outputs |
...
- Labels: `risk:high`, `tdd`, `shared-core`, `data-integrity`.
...
- Labels: `risk:medium`, `tdd`, `frontend`, `accessibility`, `decision-gate`.
...
- Labels: `risk:high`, `tdd`, `shared-core`, `api`.
...
- Labels: `risk:high`, `tdd`, `frontend`, `accessibility`, `vertical-slice`.
...
- Labels: `risk:medium`, `tdd`, `architecture`, `curriculum`.
- Depends on: I5-03; coordinates with I5-05, no portal-source overlap.
- Phase: 6.
...
- Labels: `risk:high`, `tdd`, `data-platform`, `recovery`.
- Depends on: I5-01, I5-03, I5-04.
- Phase: 7.
...
- Labels: `risk:high`, `tdd`, `performance`, `compose`.
...
- Labels: `risk:high`, `decision-gate`, `aws`, `finops`, `recovery`.
...
- Labels: `risk:high`, `tdd`, `aws`, `data-platform`, `persistence`, `no-apply`.
- Depends on: I5-09; consumes I5-10 outputs; spikes may run locally/disposable only.
- Phase: 11.
...
- Labels: `risk:high`, `tdd`, `release`, `evidence`.
...
- Labels: `risk:high`, `security:S3`, `identity`, `hosted`.
